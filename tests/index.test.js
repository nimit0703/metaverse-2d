...
describe("Authentication", () => {
  test("user be able to sign up only once", async () => {
    ...
    const updatedResponse = await axios.put(`${BACKEND_URL}/api/v1/signup`, {
      username,
      password,
      type: "admin",
    });
  ...
  test("signin succeeds if username and password are correct"),
    async () => {
      const username = "nimit" + Math.random();
      const password = "1234";
...
describe("User avatar information", () => {
  ...
  test("get back avatar information for a user", async () => {
    const response = await axios.get(
      `${BACKEND_URL}/api/v1/metadata/bult?ids=[${avatarId}]`
    );
...
describe("arena Information", () => {
  ...
  test("incorect space id return n ", async () => {
    ...
    expect(incorrectSpace.statusCode).toBe(400);
...
describe("admin endpoints", () => {
  ...
  test("user is not able to hit admin end points ", async () => {
    ...
    expect(avatarResponce.statusCode).toBe(403);
    expect(updteElementRes.statusCode).toBe(403);
  });
  ...
  test("admin is able to hit admin end points ", async () => {
    ...
    expect(avatarResponce.statusCode).toBe(200);
  });
  ...
  test("admin is able to update the imgageUrl for an element", async () => {
    ...
    expect(updteElementRes.statusCode).toBe(200);
  });
});
